...
/****** Object:  UserDefinedFunction [dbo].[ufn_Missing_DataList]    Script Date: 02/05/2016 16:44:04 ******/
...
--select * from [ufn_Missing_DataList](-1, -1, 4, -1, '01/01/1940', '01/01/1940')
		...
		(SELECT * FROM ( 
			SELECT  fu.PatientId , fu.OperationId , fu.FUPeriodId  PeriodID, ff.Description Descrip 
			        from tbl_FollowUp fu 
			              inner join tlkp_FollowUp_FUVal ff on case when fu.FUVal = 5 then 0 else fu.FUVal end = ff.id) as s
